...
function parseInventory(text) {
    ...
    const registerMatchedItem = (name, qty, raw, room, wLbs, isWeightHeavy, isSynthetic = false)=>{
        ...
        if (__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dictionaries$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["LIFT_GATE_ITEMS"].some((h)=>new RegExp(`\\b${h}\\b`, "i").test(name)) || isWeightHeavy) heavyCount += qty;
        if (__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dictionaries$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["IRREGULAR_SIGNALS"].some((s)=>new RegExp(`\\b${s}\\b`, "i").test(name))) irregularCount += qty;
        ...
        else if (__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dictionaries$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["DA_COMPLEX"].some((s)=>new RegExp(`\\b${s}\\b`, "i").test(name))) daComplexQty += qty;
        else if (__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dictionaries$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["DA_SIMPLE"].some((s)=>new RegExp(`\\b${s}\\b`, "i").test(name))) daSimpleQty += qty;
        if (!__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dictionaries$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["STRICT_NO_BLANKET_ITEMS"].some((nb)=>new RegExp(`\\b${nb}\\b`, "i").test(name))) furnitureCount += qty;
...
function buildEstimate(inputs, normalizedRows, overridesObj) {
    try {
        ...
        const hasHeavy = useNormalized ? manualHeavy : items.some((it)=>{
        ...
        if (extraStopCount > 0) {
            logs.push(`Route: ${extraStopCount} extra stop${extraStopCount > 1 ? "s" : ""}. +${extraStopMinutesTotal} min route complexity.`);
        }
        let fragileCount = 0;
        ...
        if (!isCommercial) {
            ...
            if (parsed.totalVol > 0) {
                ...
                if (parsed.boxCount === 0 && parsed.totalVol > 500) {
                    ...
                    hiddenVolume += missingBoxesCount * 5;
                    if (missingBoxesCount > 0) auditSummary.push(`Added ${missingBoxesCount} boxes (${inventoryCompleteness} commercial coverage).`);
                }
            }
        }
        ...
        if (!isLD && !isCommercial && bedroomCount >= 4 && parsed.mentionsGarageOrAttic && storageHeavyTruckPieceQty >= __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].STORAGE_HEAVY_TRUCK_MIN_PIECES) {
            ...
            auditSummary.push(`Added +${storageHeavyMixHV} cf (heavy garage/gym/storage mix).`);
        }
        ...
        if (!isLaborOnly) {
            ...
            const extraStopTruckThresholdReduction = extraStopCount > 0 ? Math.min(__TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_TRUCK_MAX_THRESHOLD_REDUCTION, extraStopCount * __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_TRUCK_THRESHOLD_REDUCTION_PER_STOP + (hasNonGroundExtraStop ? __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_TRUCK_NON_GROUND_BONUS : 0)) : 0;
            ...
            if (trucksFinal === 1 && finalVolume >= adjustedSingleTruckFloor) {
                ...
                if (extraStopTruckThresholdReduction > 0 && finalVolume < defaultSingleTruckFloor) {
                    ...
                    auditSummary.push("Extra truck added (multi-stop routing).");
                } else {
                    risks.push({
        ...
        if (!isLaborOnly && extraStopCount >= 2 && finalVolume >= __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_CREW_FLOOR_VOLUME) {
            crew = Math.max(crew, 4);
        }
        if (!isLaborOnly && extraStopComplexityScore >= __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_CREW_COMPLEXITY_HIGH && finalVolume >= __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$config$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["PROTOCOL"].EXTRA_STOP_CREW_COMPLEX_VOLUME) {
        ...
        if (extraStopCount > 0) {
            auditSummary.push(`Added +${extraStopMinutesTotal} min (${extraStopCount} extra stop${extraStopCount > 1 ? "s" : ""}${hasNonGroundExtraStop ? ", mixed access" : ""}).`);
        }
        ...
        const SAFE_DAY_LIMIT = inputs.moveType === "Local" && inputs.packingLevel === "Full" ? 11.5 : 10.5;
        ...
        if (finalVolume > 4000) {
            ...
            if (additionalCrewAvailable) advice.push(`Large Move: Recommend splitting into 2 days OR ${Math.max(6, crew + 1)}+ movers.`);
            else advice.push("Large Move: Recommend splitting into 2 days.");
        }
        ...
        if (!isCommercial && (inputs.moveType === "Local" || inputs.moveType === "LD")) {
            ...
            else if (parsed.hasVague) volumeDriver = "Volume Driver: Vague inventory increased handling allowance.";
            ...
            else if (calcDuration > SAFE_DAY_LIMIT || crewSuggestion && crew >= 3) crewDriver = "Crew Driver: Crew size was increased to keep the move within a workable day.";
        ...
        if (isCommercial && trucksFinal >= 2) advice.push("Reserve loading dock time and truck staging in advance.");
        if (isCommercial && anyHeavySignal) advice.push("Pre-measure oversized equipment, cabinets, and appliance paths.");
        if (inputs.packingLevel !== "None" && isCommercial) advice.push("Comm. Packing: Label all boxes by office/room number.");
        if (extraStopCount > 0) advice.push("Multi-stop route: confirm stop order, parking, and stop-level access before dispatch.");
        if (finalVolume > 1800 && trucksFinal === 1) advice.push("High Volume: Ensure parking spot is 40ft+ for large truck maneuvering.");
        advice.forEach((a)=>{
            if (splitRecommended && a.includes("2-Day Split")) return;
            if (!seenAdvice.has(a)) {
                seenAdvice.add(a);
                uniqueAdvice.push(a);
            }
...
//# sourceMappingURL=_81de7692._.js.map