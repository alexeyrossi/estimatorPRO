...
export function buildEstimate(inputs: EstimateInputs, normalizedRows?: NormalizedRow[], overridesObj?: Record<string, string>): EstimateResult {
  try {
    ...
    const needsStorageHeavyTruckFloor =
      inputs.moveType === "Storage" &&
      !isCommercial &&
      bedroomCount >= 4 &&
      parsed.mentionsGarageOrAttic &&
      storageHeavyTruckPieceQty >= PROTOCOL.STORAGE_HEAVY_TRUCK_MIN_PIECES &&
      finalVolume >= Math.floor(PROTOCOL.TRUCK_CAPACITY_SAFE * PROTOCOL.STORAGE_HEAVY_TRUCK_FLOOR_RATIO);
    ...
    if (inputs.packingLevel === "Full") { packingAddonMH = (totalBoxes * PROTOCOL.MINS_PACK_BOX) / 60; }
    ...
    advice.forEach(a => { if (splitRecommended && a.includes("2-Day Split")) return; if (!seenAdvice.has(a)) { seenAdvice.add(a); uniqueAdvice.push(a); } });
