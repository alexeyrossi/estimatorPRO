...
function parseInventory(text) {
    ...
    tokens.forEach(({ text: tok, room }) => {
        ...
        if (matchedKey) {
            matchedAny = true;
            const cf = dictionaries_1.VOLUME_TABLE[matchedKey] * qty;
            totalVol += cf;
            ...
            if (new RegExp(`\\b(box|bin|tote)s?\\b`, 'i').test(matchedKey))
                boxCount += qty;
            if (dictionaries_1.LIFT_GATE_ITEMS.some(h => new RegExp(`\\b${h}\\b`, 'i').test(matchedKey)) || isWeightHeavy)
                heavyCount += qty;
            if (dictionaries_1.IRREGULAR_SIGNALS.some(s => new RegExp(`\\b${s}\\b`, 'i').test(matchedKey)))
                irregularCount += qty;
            ...
            if (isBedUnit)
                daComplexQty += qty;
            else if (dictionaries_1.DA_COMPLEX.some(s => new RegExp(`\\b${s}\\b`, 'i').test(matchedKey)))
                daComplexQty += qty;
            else if (dictionaries_1.DA_SIMPLE.some(s => new RegExp(`\\b${s}\\b`, 'i').test(matchedKey)))
                daSimpleQty += qty;
            if (!dictionaries_1.STRICT_NO_BLANKET_ITEMS.some(nb => new RegExp(`\\b${nb}\\b`, 'i').test(matchedKey)))
                furnitureCount += qty;
            else
                noBlanketVol += cf;
        }
